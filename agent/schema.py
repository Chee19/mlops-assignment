    ...
    with sqlite3.connect(f"file:{path}?mode=ro", uri=True) as conn:
        ...
        for t in tables:
            ...
            for fk in conn.execute(f"PRAGMA foreign_key_list({_q(t)})"):
                ...
                # `to` is NULL when the FK implicitly references the parent's PK;
                # fall back to the child column name so rendering never crashes.
                col_lines.append(
                    f"  FOREIGN KEY ({_q(fk[3])}) REFERENCES {_q(fk[2])}({_q(fk[4] or fk[3])})"
                )
            parts.append(",\n".join(col_lines))
            parts.append(");")
    return "\n".join(parts)
